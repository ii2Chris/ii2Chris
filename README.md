# Hey, I'm Chris! 👋

A **full-stack developer** who loves building clean, user-focused web applications.  
📂 Portfolio → [chrisbudhram.vercel.app](https://chrisbudhram.vercel.app)

---

## 🧰 Languages and Tools
HTML • CSS • JavaScript • React • Node.js • Express • C++ • TailwindCSS • Git/GitHub • Neovim • VSCode

---

## 📖 Currently Learning
- AI/ML integration  
- Advanced React patterns  
- Tailwind CSS  

---

## 🚀 Projects
- [**Responsive Grid Layout**](https://responsive-grid-layout-mu.vercel.app/) — Adaptive grid demo (desktop/tablet/mobile).  
- [**CertGram**](https://certgram.vercel.app/) — Concert-matching platform built with React + Node.  
- [**Portfolio Website**](https://chrisbudhram.vercel.app/) — My personal developer portfolio.  

---

## 📬 Let's Connect
📧 **Email:** chrisbudhram03@outlook.com  
💼 **LinkedIn:** [linkedin.com/in/chrisbudhram](https://www.linkedin.com/in/chrisbudhram)  
🌐 **Portfolio:** [chrisbudhram.vercel.app](https://chrisbudhram.vercel.app)  

---

## 🎮 Fun Fact
Probably better at [Marvel Rivals](https://tracker.gg/marvel-rivals/profile/ign/iiCopy/overview) than I am at coding... probably.  

---

*"Building simple, beautiful user experiences — one commit at a time."*
